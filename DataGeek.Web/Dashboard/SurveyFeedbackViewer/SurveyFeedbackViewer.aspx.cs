...
public partial class SurveyFeedbackViewer : System.Web.UI.Page
{
    ...
    {
        String qry = "SELECT t.*, (SELECT LTRIM(GROUP_CONCAT(CONCAT(' ',MagazineName))) " +
        "FROM db_surveysubscriptions s " +
        "WHERE s.SurveyFeedbackID = t.SurveyFeedbackID) AS Magazines " +
    ...
    protected void SetSurveyViewCount()
    {
        ...
        qry = "SELECT COUNT(*) as 'c' FROM db_surveyfeedbackviews WHERE ViewedBy LIKE '%Unknown%'"; // not for BizClik employees
        String num_views = SQL.SelectString(qry, "c", null, null);
        
        lbl_view_stats.Text = "Survey Feeback form has been viewed anonymously a total of " + Server.HtmlEncode(num_views.ToString())
    ...
    protected void gv_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            lb.OnClientClick = "window.open('/feedbacksurvey.aspx?v_id=" + Server.UrlEncode(e.Row.Cells[2].Text) + "&lang=" + Server.UrlEncode(e.Row.Cells[4].Text)
