    ...
    public partial class SmartSocialTracker : System.Web.UI.Page
    {
        ...
        private void BindFeatures()
        {
            if (dd_issue.Items.Count > 0 && dd_issue.SelectedItem != null && dd_region.Items.Count > 0 && dd_region.SelectedItem != null)
            {
                ...
                String qry = "SELECT et.CompanyID, et.Region, cpy.CompanyName as 'feature', Writer, " +
                "CASE WHEN eti.IssueName!=@issue THEN 1 ELSE 0 END as 'rr', " +
                "CASE WHEN eti.IssueName!=@issue THEN rr.CompanyType ELSE et.CompanyType END as 'type', " +
                "CASE WHEN eti.IssueName!=@issue THEN rr.IsCancelled ELSE et.IsCancelled END as 'cancelled', " +
                "SharedOnTwitter,SharedOnFacebook,SharedOnLinkedin,SharedOnWebsite,SharedOnOther, " +
                ...
                "LEFT JOIN db_editorialtrackerreruns rr ON et.EditorialID = rr.EditorialID AND rr.EditorialTrackerIssueID IN (SELECT EditorialTrackerIssueID FROM db_editorialtrackerissues WHERE IssueName=@issue) " +
                "LEFT JOIN db_smartsocialpage ssp ON ssp.CompanyID = et.CompanyID AND ssp.MagazineIssueName=@issue " +
                "LEFT JOIN db_smartsocialstatus sss ON sss.CompanyID = et.CompanyID AND sss.Issue=@issue " +
                "WHERE (eti.IssueName=@issue OR et.EditorialID IN (SELECT EditorialID FROM db_editorialtrackerreruns WHERE EditorialTrackerIssueID IN(SELECT EditorialTrackerIssueID FROM db_editorialtrackerissues WHERE IssueName=@issue))) " +
                "AND et.Region=@region " +
                "AND CASE WHEN eti.IssueName != @issue THEN rr.IsCancelled ELSE et.IsCancelled END = 0 " + // not cancelled
                "AND CASE WHEN eti.IssueName != @issue THEN rr.CompanyType ELSE et.CompanyType END != 1 " + // not parachutes
                "ORDER BY cpy.CompanyName";
        ...
        protected void rg_features_ItemDataBound(object sender, GridItemEventArgs e)
        {
            if (e.Item is GridNestedViewItem)
            {
                ...
                String cpy_id = ((HiddenField)nested_item.FindControl("hf_cpy_id")).Value;
                ...
                {
                    iframe.Attributes.Add("src", "trackeradvertiserlist.aspx?feat_cpy_id=" + cpy_id + "&issue_name=" + dd_issue.SelectedItem.Text);
            ...
            else if (e.Item is GridDataItem)
            {
                ...
                String cpy_id = item["cpy_id"].Text;
