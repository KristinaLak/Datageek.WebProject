...
public partial class AppointmentManager : System.Web.UI.Page
{
    ...
    private void ConfigureForm()
    {
        ...
        String qry = "SELECT TRIM(CONCAT(CASE WHEN first_name IS NULL THEN '' ELSE CONCAT(TRIM(first_name), ' ') END, CASE WHEN last_name IS NULL THEN '' ELSE CONCAT(TRIM(last_name)) END)) as 'ContactName', " +
        "company_name, email, personal_email " +
        "FROM dbl_lead, db_contact, db_company WHERE dbl_lead.ContactID=db_contact.ctc_id AND db_contact.new_cpy_id=db_company.cpy_id AND LeadID=@LeadID";
        DataTable dt_lead_info = SQL.SelectDataTable(qry, "@LeadID", hf_lead_id.Value);
        if (dt_lead_info.Rows.Count > 0)
        {
            String ContactName = dt_lead_info.Rows[0]["ContactName"].ToString();
            String CompanyName = dt_lead_info.Rows[0]["company_name"].ToString();
            ...
            //if (dt_lead_info.Rows[0]["personal_email"].ToString() != String.Empty && Util.IsValidEmail(dt_lead_info.Rows[0]["personal_email"].ToString()))
            //    Attendees += dt_lead_info.Rows[0]["personal_email"].ToString() + "; ";
